    ...
    while (!pending_nodes.empty() && rclcpp::ok()) {
        ...
        for (const auto & dir : explore_directions) {
            ...
            if (std::find(visited_nodes.begin(), visited_nodes.end(), new_node) == visited_nodes.end() &&
                poseInMap(new_node) && map_->data.at(poseToCell(new_node)) < 99 &&
...
double AStarPlanner::manhattanDistance(const GraphNode & a, const GraphNode & b)
{
    return std::abs(a.x - b.x) + std::abs(a.y - b.y);
}

bool AStarPlanner::poseInMap(const GraphNode & node)
{
    return node.x >= 0 && node.x < static_cast<int>(map_->info.width) &&
           node.y >= 0 && node.y < static_cast<int>(map_->info.height);
}

int AStarPlanner::poseToCell(const GraphNode & node)
{
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<path_planning::AStarPlanner>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}